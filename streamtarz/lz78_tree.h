...
public:
    ...
    void dfs(int &now, UC val, int i) {
        int here = ch[now].node[val];
        if (last[here] == i) {
            int length = len(pa[here]);
            fwrite(&length, sizeof('t'), 1, stdout);
            fwrite(&pa[here], sizeof('t') * length, 1, stdout);
    ...
    void writeinfo() {
        ...
        if (last[now] != V.size() - 1) {
            ...
            int length = len(now);
            fwrite(&length, sizeof('t'), 1, stdout);
            fwrite(&now, sizeof('t') * length, 1, stdout);
            fwrite(&chr, sizeof('l'), 1, stdout);
        }
        return;
    }
};
